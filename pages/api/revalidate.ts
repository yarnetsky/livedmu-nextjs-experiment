/**
 * This code is responsible for revalidating the cache when a story or author is updated.
 *
 ...
 * 6. Filter: _type == "story" || _type == "author" || _type == "settings"
...
type StaleRoute = '/' | `/stories/${string}`
...
async function queryStaleRoutes(
  ...
  if (body._type === 'story') {
    ...
    if (!exists) {
      ...
      if ((body.slug as any)?.current) {
        staleRoutes.push(`/stories/${(body.slug as any).current}`)
      }
      // Assume that the story document was deleted. Query the datetime used to sort "More stories" to determine if the story was in the list.
      const moreStories = await client.fetch(
        groq`count(
          *[_type == "story"] | order(date desc, _updatedAt desc) [0...3] [dateTime(date) > dateTime($date)]
        )`,
        { date: body.date },
      )
      // If there's less than 3 stories with a newer date, we need to revalidate everything
      if (moreStories < 3) {
  ...
  switch (body._type) {
    ...
    case 'story':
      return await queryStaleStoryRoutes(client, body._id)
    case 'settings':
...
async function mergeWithMoreStories(
  ...
  const moreStories = await client.fetch(
    groq`*[_type == "story"] | order(date desc, _updatedAt desc) [0...3].slug.current`,
  )
...
async function queryStaleAuthorRoutes(
  ...
  let slugs = await client.fetch(
    ...
    "slug": *[_type == "story" && references(^._id)].slug.current
  }["slug"][]`,
    { id },
  )
  ...
  if (slugs.length > 0) {
    ...
    return ['/', ...slugs.map((slug) => `/stories/${slug}`)]
  }

  return []
}

async function queryStaleStoryRoutes(
  client: SanityClient,
  id: string,
): Promise<StaleRoute[]> {
  let slugs = await client.fetch(
    groq`*[_type == "story" && _id == $id].slug.current`,
    { id },
  )
